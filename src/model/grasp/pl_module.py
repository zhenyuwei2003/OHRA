...
from model.grasp.diffusion_model import DiffusionModel
from model.grasp.encoder import Encoder
from model.vae import VAE


def create_params_encoder(cfg):
    params_encoder = VAE(
        input_dim=cfg.input_dim,
        hidden_dims=cfg.hidden_dims,
        latent_dim=cfg.latent_dim,
    )
    ckpt_path = f"{ROOT_DIR}/output/{cfg.ckpt_name}/state_dict/epoch={cfg.ckpt_epoch}.ckpt"
    ...
    params_encoder.load_state_dict(state_dict)
...
class TrainingModule(pl.LightningModule):
    def __init__(self, cfg):
        ...
        self.params_encoder = create_params_encoder(cfg.model.params_encoder)
    ...
    def training_step(self, batch, batch_idx):
        ...
        hand_params_feat = self.params_encoder.reparameterize(*self.params_encoder.encode(hand_params)).detach()
        object_pc_feat = self.pc_encoder(object_pc)
        feat = torch.cat([object_pc_feat.mean(dim=1), hand_params_feat, q_rot], dim=1)
    ...
    def inference(self, object_pc, hand_params, rot6d):
        mu, logvar = self.params_encoder.encode(hand_params)
        hand_params_feat = self.params_encoder.reparameterize(mu, logvar)
        object_pc_feat = self.pc_encoder(object_pc)
        feat = torch.cat([object_pc_feat.mean(dim=1), hand_params_feat, rot6d], dim=1)
