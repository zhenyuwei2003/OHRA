...
class HandModel:
    def __init__(self, robot_name, urdf_type, urdf_file, is_canonical, is_extended=False, use_collision=False, device=torch.device('cpu')):
        ...
        self.meta_info = json.load(open(os.path.join(ROOT_DIR, f"assets/meta_infos/{'extended' if self.is_extended else 'base'}/{self.robot_name}.json")))
    ...
    def get_canonical_ordered_q(self, q, canonical_order=None):
        ...
        if not self.is_extended:  # base canonical model only adjustment
