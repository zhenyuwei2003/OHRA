...
def vis_hand(viser_server, hand_model, q, color=(102, 192, 255), use_trimesh_vis=False, print_frame_info=False):
    ...
    for link_name, transform in hand_model.frame_status.items():
        ...
        if print_frame_info:
            print(f"Frame {link_name} | {joint_name}: {matrix[:3, 3]}")
...
def main(args):
    ...
    meta_info = json.load(open(os.path.join(ROOT_DIR, f"assets/meta_infos/{'extended' if args.extended else 'base'}/{args.robot_name}.json")))
    ...
    vis_hand(server, hand, initial_q, print_frame_info=True)
    slider_list = []
