...
def isaac_main(
    ...
    urdf_path = os.path.join(ROOT_DIR, "grasp_zeroshot/assets/urdf")
    hand = HandModel(
    ...
    simulator.set_asset(
        ...
        object_path=os.path.join(ROOT_DIR, f"data/grasp_zeroshot/object/"),
        object_file=f"{object_name}.urdf"
    )
    simulator.create_envs()
    print("[IsaacGym] IsaacValidator preparation is done.")

    if mode == 'filter':
        q = torch.load(os.path.join(ROOT_DIR, f"data/grasp_zeroshot/tmp/{robot_name}/{object_name}_{batch_size}.pt"))
...
if __name__ == '__main__':
    ...
    if args.mode == 'filter':
        ...
        if success_num > 0:
            ...
            save_dir = str(os.path.join(ROOT_DIR, f"data/grasp_zeroshot/tmp/{args.robot_name}"))
