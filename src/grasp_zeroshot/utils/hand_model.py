...
def parse_urdf_to_trimesh(urdf_type, urdf_file, link_names=None, use_collision=False):
    ...
    for link in root.findall('link'):
        ...
        if meshes:
            if len(meshes) == 1 and isinstance(meshes[0], trimesh.Trimesh):
                ...
                try:
                    ...
                    print(f"Boolean union failed for link {link_name}: {e}")
...
class HandModel:
    def __init__(self, robot_name, urdf_type, urdf_file, is_canonical, is_extended=False, use_collision=False, device=torch.device('cpu')):
        ...
        with open(os.devnull, 'w') as f, contextlib.redirect_stderr(f):  # redirect stderr to remove parse warnings
    ...
    def get_canonical_ordered_q(self, q, canonical_order):
        assert self.is_canonical, "Conversion is required when not using a canonical hand model."
        assert len(q) == 16, f"Expected 16 DoF for {self.robot_name}, got {len(q)}"

        q_canonical = [np.sign(x) * q[abs(x) - 1] for x in canonical_order]
        return q_canonical

    def get_original_ordered_q(self, q_canonical, canonical_order):
        ...
        q = [0.0] * 16
        for canonical_idx, x in enumerate(canonical_order):
            if x != 0:
